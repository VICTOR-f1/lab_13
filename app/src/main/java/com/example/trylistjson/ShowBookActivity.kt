...
class ShowBookActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var genresget = moneyDAO.getAllGenre()
        ...
        genresget.observe(this,androidx.lifecycle.Observer {
            BookList.addAll(it)
            Log.d("QWE", "its ok!")
  ...
  /*  private fun updeteInfo() {
        ...
        val gab = genrebase?.bookDao()?.getAllBookMain()

        gab?.observe(this, androidx.lifecycle.Observer {
            Log.d("QWE", "asd")
        })

        genrebase?.bookDao()?.getAllBookMain()?.observe(this, androidx.lifecycle.Observer {
            BookList.addAll(it)

        })

        Toast.makeText(this, "gsdfsdfddsf", Toast.LENGTH_SHORT).show()
    }*/
}
